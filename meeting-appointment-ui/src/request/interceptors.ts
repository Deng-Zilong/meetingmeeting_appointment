...
export function ResResolve(res: any) {
  ...
  // 验证码不判断状态码
  if (res.config.url === "/meeting/user/captcha.jpg" || res.config.url === `/meeting/meeting-record/record-export/${userId}/0/0` || res.config.url === `/meeting/meeting-record/record-export/${userId}/1/0`) {
    return res;
  }
